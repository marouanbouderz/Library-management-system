...
<div class="mb-6 flex items-center gap-4 p-4 bg-red-50 border border-red-200 rounded-2xl">
    ...
    <div class="flex-1">
        <p class="text-sm font-bold text-red-800">
            {{ $totalAlerts }} {{ Str::plural('alert', $totalAlerts) }} require your attention
        </p>
        <p class="text-xs text-red-600 mt-0.5">
            @if($overdueCount > 0) {{ $overdueCount }} overdue @endif
            @if($overdueCount > 0 && $warningCount > 0) · @endif
            @if($warningCount > 0) {{ $warningCount }} expiring soon @endif
        </p>
    </div>
</div>
@endif
                ...
                <div class="flex items-start justify-between gap-3">
                    <div>
                        ...
                        <p class="text-xs text-slate-500 mt-1">
                            <span class="font-mono font-semibold text-slate-700">{{ $row->Book_ID }}</span>
                            @if($book) <span class="text-slate-400">·</span> {{ $book->Book_Name }}@endif
                        </p>
                    </div>
