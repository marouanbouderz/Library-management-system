...
@section('title', 'Supprimer étudiant')
@section('content')

<div class="flex items-center justify-between mb-6">
    <div>
        <h2 class="text-xl font-bold text-slate-800">Supprimer étudiant</h2>
        <p class="text-sm text-slate-500 mt-0.5">Permanently remove a student from the system</p>
    </div>
    <nav class="flex items-center gap-2 text-xs text-slate-400">
        <a href="{{ url('admin/dashboard') }}" class="hover:text-[#4F6FCD] transition-colors">Dashboard</a>
        <i class="fas fa-chevron-right text-[10px]"></i>
        <span class="text-slate-600 font-medium">Supprimer étudiant</span>
    </nav>
</div>
...
<div class="mb-5 flex items-center gap-3 p-4 rounded-xl" style="background:#fff1f2; border:1px solid #fecdd3;">
    ...
    <p class="text-sm font-semibold" style="color:#991b1b;">Removing a student is permanent and cannot be undone. Ensure the student has no active Emprunts actifs before proceeding.</p>
</div>
...
<div class="bg-white rounded-2xl shadow-md overflow-hidden">
    <div class="px-6 py-4 flex items-center gap-3" style="border-bottom:1px solid #E8EDF2;">
        ...
        <div>
            <h3 class="font-bold text-slate-800 text-sm">Student List</h3>
            <p class="text-xs text-slate-400">Select a student to remove permanently</p>
        </div>
    </div>
        ...
        <table id="dataTable" class="w-full text-sm">
            <thead>
                <tr style="background:#F8FAFC;">
                    ...
                    <th class="text-left px-4 py-3 text-xs font-bold uppercase tracking-wider" style="color:#6B7280;">Emprunts actifs</th>
            ...
            <tbody>
                ...
                @php $available_student_copy = DB::table('records')->where('Student_ID',$row->Student_ID)->where('Submission_Status','No')->count(); @endphp
                <tr>
                    <td class="px-4 py-4 text-slate-400 font-semibold">{{ str_pad($count, 2, '0', STR_PAD_LEFT) }}</td>
                    <td class="px-4 py-4">
                        <div class="flex items-center gap-3">
                            <img src="{{ asset($row->Image ?: 'image/default.svg') }}" class="w-9 h-9 rounded-full object-cover border-2 border-slate-100" alt="{{ $row->Name }}" onerror="this.src='{{ asset('image/default.svg') }}'">
                            <span class="font-bold text-slate-800">{{ $row->Name }}</span>
                        </div>
                    </td>
                    <td class="px-4 py-4 font-mono text-slate-600 text-xs">{{ $row->Student_ID }}</td>
                    <td class="px-4 py-4"><span class="bg-slate-100 text-slate-600 text-xs font-bold px-2.5 py-1 rounded-full">{{ $row->Session }}</span></td>
                    ...
                    <td class="px-4 py-4">
                        @if($available_student_copy > 0)
                            <span class="inline-flex items-center gap-1 text-xs font-bold px-2.5 py-1 rounded-full" style="background:#fee2e2; color:#dc2626;">
                                <i class="fas fa-exclamation-circle text-[10px]"></i> {{ $available_student_copy }} active
                            </span>
                        @else
                            <span class="text-xs font-bold px-2.5 py-1 rounded-full" style="background:#D1FAE5; color:#065F46;">Clear</span>
                        @endif
                    </td>
                    <td class="px-4 py-4">
                        <a href="{{ url('admin/student/Supprimer/process/'.$row->id) }}"
                           onclick="return Confirmer('Permanently remove {{ $row->Name }}? This cannot be undone.')"
