...
@section('title', 'Livres — Génie Civil')
@section('content')

<div class="flex items-center justify-between mb-6">
    <div>
        <h2 class="text-xl font-bold text-slate-800">Livres — Génie Civil</h2>
        <p class="text-sm text-slate-500 mt-0.5">Browse the civil engineering category collection</p>
    </div>
    <div class="flex items-center gap-3">
        <nav class="flex items-center gap-2 text-xs text-slate-400">
            <a href="{{ url('admin/dashboard') }}" class="hover:text-[#4F6FCD] transition-colors">Dashboard</a>
            <i class="fas fa-chevron-right text-[10px]"></i>
            <span class="text-slate-600 font-medium">Génie Civil</span>
        </nav>
...
<div class="bg-white rounded-2xl shadow-md overflow-hidden">
    <div class="px-6 py-4 flex items-center gap-3" style="border-bottom:1px solid #E8EDF2;">
        <div class="w-9 h-9 rounded-xl flex items-center justify-center" style="background:rgba(8,145,178,0.09);">
            <i class="fas fa-drafting-compass" style="color:#0891b2;"></i>
        </div>
        <div>
            <h3 class="font-bold text-slate-800 text-sm">Collection Génie Civil</h3>
            <p class="text-xs text-slate-400">Structures, béton, topographie et travaux publics</p>
        </div>
    </div>
        ...
        <table id="dataTable" class="w-full text-sm">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="px-4 py-4">
                        @if($available > 0)
                            <span class="text-xs font-bold px-2.5 py-1 rounded-full" style="background:#CFFAFE; color:#0e7490;">{{ $available }} left</span>
                        @else
                            <span class="text-xs font-bold px-2.5 py-1 rounded-full" style="background:#fee2e2; color:#dc2626;">Épuisé</span>
                        @endif
                    </td>
                ...
                <tr>
                    <td colspan="7" class="px-6 py-20 text-center">
                        <div class="flex flex-col items-center gap-4">
                            <div class="w-20 h-20 rounded-2xl flex items-center justify-center" style="background:#ECFEFF;">
                                <i class="fas fa-drafting-compass text-4xl" style="color:#a5f3fc;"></i>
                            </div>
                            <p class="font-bold text-slate-500">Aucun livre Génie Civil trouvé</p>
                            <a href="{{ url('admin/add-book') }}" class="text-xs font-semibold hover:underline" style="color:#4F6FCD;">Ajouter un livre</a>
                        </div>
                    </td>
                </tr>
                @endforelse
