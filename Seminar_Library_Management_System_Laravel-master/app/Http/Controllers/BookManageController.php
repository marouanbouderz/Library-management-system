...
class BookManageController extends Controller
{
    ...
    public function add_order_process(Request $req)
    {
        ...
        $expiredDate = date('d-m-Y', strtotime("+14 days", strtotime($today)));
    ...
    public function civile_book()
    {
        $admin_status = Session::get('Admin_ID');
        if (!$admin_status) return Redirect::to('/');

        $book = DB::table('books')->where('Catagory', 'Civile')->get();
        return view('admin.civile_book', compact('book'));
    }
    ...
    public function civile_book_student()
    {
        $student_status = Session::get('Student_ID');
        if (!$student_status) return Redirect::to('/');

        $student = DB::table('students')->where('id', $student_status)->get();
        $book    = DB::table('books')->where('Catagory', 'Civile')->get();
        return view('student.civile_book', compact('student', 'book'));
    }
    ...
    public function catalogue()
    {
        ...
        $student    = DB::table('students')->where('id', $student_status)->get();
        $books      = DB::table('books')->orderBy('Catagory')->get();
        $categories = DB::table('books')->distinct()->orderBy('Catagory')->pluck('Catagory');

        return view('student.catalogue', compact('student', 'books', 'categories'));
    }
    ...
    public function borrow_book($book_id)
    {
        ...
        if (!$book) {
            return back()->with(['message' => 'Livre introuvable.', 'alert-type' => 'error']);
        }

        if ($book->Amounts <= 0) {
            return back()->with(['message' => 'Ce livre n\'est plus disponible.', 'alert-type' => 'error']);
        }

        $already = DB::table('records')
        ...
        if ($already > 0) {
            return back()->with(['message' => 'Vous avez déjà emprunté ce livre.', 'alert-type' => 'error']);
        }
        ...
        $inserted = DB::table('records')->insert([
        ...
        if ($inserted) {
            DB::table('books')->where('Book_ID', $book->Book_ID)->decrement('Amounts');
            return back()->with(['message' => 'Livre emprunté ! À retourner avant le ' . $expiredDate . '.', 'alert-type' => 'success']);
        }

        return back()->with(['message' => 'Erreur lors de l\'emprunt.', 'alert-type' => 'error']);
    }
