...
class StudentManageController extends Controller
{
    ...
    public function borrow_history()
    {
        ...
        $studentRow = DB::table('students')->where('id', $student_status)->first();

        $history = DB::table('records')
        ...
        return view('student.borrow_history', compact('student', 'history'));
    }

    public function my_fines()
    {
        $student_status = Session::get('Student_ID');
        if (!$student_status) return Redirect::to('/');
        ...
        return view('student.my_fines', compact('student', 'records'));
    }

    public function admin_fines()
    {
        $admin_status = Session::get('Admin_ID');
        ...
        return view('admin.fines', compact('records'));
    }

    public function mark_fine_paid($id)
    {
        $admin_status = Session::get('Admin_ID');
        if (!$admin_status) return Redirect::to('/');

        DB::table('records')->where('id', $id)->update(['Fine_Paid' => 'Yes']);

        return back()->with(['message' => 'Amende marquée comme payée.', 'alert-type' => 'success']);
    }
